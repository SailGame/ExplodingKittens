...
TEST_F(ContainerFixture, CreateGame) {
    ...
    EXPECT_CALL(mMockCardPool, Back()).WillRepeatedly(testing::Return(GetBottom));
    ...
    EXPECT_EQ(currentPlayer->mCards.size(),4);
    EXPECT_CALL(mMockCardPool, Front()).WillRepeatedly(testing::Return(Shuffle));
    ...
    currentPlayer->process_event(PlayCardShirk(mGame.mPlayers[ShirkTargetIndex].mUid));
    EXPECT_EQ(mGame.mPlayingPlayerPos,0);
